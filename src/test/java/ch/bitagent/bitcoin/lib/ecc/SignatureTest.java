...
class SignatureTest {
    ...
    void der() {
        ...
        for (int i = 0; i < 2; i++) {
            r = Hex.parse(Bytes.randomBytes(Int.parse(2).pow(Int.parse(256)).toBytes(32).length));
            s = Hex.parse(Bytes.randomBytes(Int.parse(2).pow(Int.parse(255)).toBytes(32).length));
